...
namespace Tidy.Core
{
    ...
    {
        private readonly TagCollection _tt;
		...
		*/
        ...
        {
            StyleProp prop = CreateProps(null, style);
            prop = CreateProps(prop, property);
            style = CreatePropString(prop);
            return style;
        }

        private string GenSymClass()
        {
            string str = "c" + _classNum;
            _classNum++;
            return str;
        }
		...
		*/
        ...
        {
            StyleProp prop = CreateProps(null, s1);
            prop = CreateProps(prop, s2);
            string s = CreatePropString(prop);
            return s;
        }
        ...
        {
            var sizes = new[] {"60%", "70%", "80%", null, "120%", "150%", "200%"};
            string buf;
		...
		*/
        ...
        private void TextAlign(Node node)
        {
            AttVal av;
            AttVal prev = null;
		...
		*/
        ...
        private bool Dir2Div(Node node)
        {
		...
		*/
        ...
        private bool MergeDivs(Node node)
        {
		...
		*/
        ...
        private bool BlockStyle(Node node)
        {
            if ((node.Tag.Model & (ContentModel.BLOCK | ContentModel.LIST | ContentModel.DEFLIST | ContentModel.TABLE)) !=
                0)
            {
		...
		*/
        ...
        public virtual void PurgeAttributes(Node node)
        {
            ...
            while (attr != null)
            {
                AttVal next = attr.Next;

                /* special check for class="Code" denoting pre text */
                if (attr.Attribute != null && attr.Val != null && attr.Attribute.Equals("class") &&
                    attr.Val.Equals("Code"))
                {
                    prev = attr;
                }
                else if (attr.Attribute != null &&
                         (attr.Attribute.Equals("class") || attr.Attribute.Equals("style") ||
                          attr.Attribute.Equals("lang") || attr.Attribute.StartsWith("x:") ||
                          ((attr.Attribute.Equals("height") || attr.Attribute.Equals("width")) &&
                           (node.Tag == _tt.TagTd || node.Tag == _tt.TagTr || node.Tag == _tt.TagTh))))
                {
                    if (prev != null)
                    {
                        prev.Next = next;
